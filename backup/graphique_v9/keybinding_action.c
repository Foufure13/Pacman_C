...
{
    if ( rect_image->directionY == 1 && rect_image->vitesseY == 1 && (map->tableau[pacman_dstrect->y+16][pacman_dstrect->x]  == '-' || map->tableau[pacman_dstrect->y+16][pacman_dstrect->x]  == '|')){
        rect_image->vitesseY = 0 ;
    }
    if ( rect_image->directionY == -1 && rect_image->vitesseY == 1 && (map->tableau[pacman_dstrect->y-16][pacman_dstrect->x]  == '-' || map->tableau[pacman_dstrect->y-16][pacman_dstrect->x]  == '|')){
        rect_image->vitesseY = 0 ;
    }
    if ( rect_image->directionX == 1 && rect_image->vitesseX == 1 && (map->tableau[pacman_dstrect->y][pacman_dstrect->x+16]  == '|' || map->tableau[pacman_dstrect->y][pacman_dstrect->x+16]  == '-')){
