...
int main(int argc, char** argv)
{
    ...
    SDL_Rect srcrect = { 16, 16, 15, 15 };
    SDL_Rect dstrect = { 600, 600, 15, 15 };
...
initialisation_fenetre();
    ...
    if (SDL_RenderCopy(rendu, texture, &srcrect, &dstrect) !=0)
    ...
    while (programme)
    {
        ...
        dstrect.y += rect_image.vitesseY *  rect_image.directionY ;
        dstrect.x += rect_image.vitesseX *  rect_image.directionX ;

        stop_border_screen(dstrect, windowHauteur, windowLargeur, &rect_image);
        ...
        SDL_RenderCopy(rendu, texture,  &srcrect,&dstrect);

        // // Afficher le rendu
        SDL_RenderPresent(rendu);
        

        appuye_touche(event, &programme, &rect_image, &srcrect);

        // std::cout << "Delta Time: " << deltaTime << std::endl;
        // std::cout << "FPS: " << 60.0 - deltaTime << std::endl;

        SDL_Delay(1000.0f / (180.0 - deltaTime));
    }
