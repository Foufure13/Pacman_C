...
void collision_fantom_pacman(MAPS *map, SDL_Rect *pacman_dstrect, Deplacement *pacman,  SDL_Rect *fantom_dstrect )
{
    for (int y =-8; y <14; y++) {
    for (int x =-8; x <14; x++) {
        
    if ((pacman_dstrect->y +y == fantom_dstrect->y && pacman_dstrect->x +x == fantom_dstrect->x) || (pacman_dstrect->y  == fantom_dstrect->y +y && pacman_dstrect->x == fantom_dstrect->x +x) ) {
        pacman->vie +=1;
    }
    }
    }
