...
void allocation_memoire_tableau(MAPS* map)
{
  ...
  int nombre_point_hauteur = map->y /16;
  int nombre_point_largeur = map->x /16;
  map->pointMap = malloc(nombre_point_hauteur * sizeof(int*));
  for (int i = 0; i < nombre_point_hauteur; i++) {
    map->pointMap[i] = malloc(nombre_point_largeur * sizeof(int));
  }
...
void ajout_structur_map(MAPS *maps, SDL_Window *fenetre, SDL_Renderer *rendu)
{
...
void création_point_map(MAPS *maps, SDL_Window *fenetre, SDL_Renderer *rendu)
{

    SDL_Surface *point = SDL_LoadBMP("assets/strucure.bmp");
    SDL_Texture *point_texture = SDL_CreateTextureFromSurface(rendu,point); ;
    SDL_Rect point_srcrect = { 0, 0, 4, 4 };
    SDL_Rect point_dstrect = { 0, 0, 4, 4 };
...
void ajout_point_map(MAPS *maps, SDL_Window *fenetre, SDL_Renderer *rendu)
{
      ...
      for(int x =0; x < maps->x; x+=16) {
        ...
        if (y > 16 && y < maps->y-48 && x > 16 && x < maps->x -16-16) {
          ...
          if (maps->tableau[y][x] != 'M'){
                ...
                point_dstrect.x = x+5;
                point_dstrect.y = y+5;
                SDL_RenderCopy(rendu, point_texture, &point_srcrect, &point_dstrect);
...
void nettoyage_pointMap(MAPS *maps)
{
    int nombre_point_hauteur = maps->y /16 ;
    // Libérer la mémoire allouée pour le tableau seedmap
    for (int i = 0; i < nombre_point_hauteur; i++) {
      free(maps->pointMap[i]);
    }
    free(maps->pointMap);
